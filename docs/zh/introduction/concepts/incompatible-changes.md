...
固定桶 Sink 现在强制同一物理分区和 bucket 由单个 Writer 写入。请设置 `multi_table_sink_replica = 1`，拆开固定桶与其他桶模式的 Sink 定义；多个源流指向同一物理表时，应先合并再写入一个 Sink。此前接受的冲突配置现在会在初始化时失败。路由 Writer 拒绝在线结构变更，需要使用更新后的 schema 重建任务。Checkpoint 序列化格式不变，本修复不提供额外的全局恢复或扩缩容保证。
