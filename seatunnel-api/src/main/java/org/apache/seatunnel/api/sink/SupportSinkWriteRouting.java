...
public interface SupportSinkWriteRouting {
     ...
     * implementation may initialize writer ownership here and must reject unsupported layouts
     * instead of silently falling back to default routing. Wrappers must expose their delegates'
     * routing policies and validate that they can be combined safely.
     */
    Optional<SinkWriteRouting> getWriteRouting();
     ...
     */
    static Optional<SinkWriteRouting> resolve(SeaTunnelSink<?, ?, ?, ?> sink) {
        return sink instanceof SupportSinkWriteRouting
                ? ((SupportSinkWriteRouting) sink).getWriteRouting()
                : Optional.empty();
    }
}
